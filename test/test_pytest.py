...
@pytest.mark.parametrize('json_object', [
    '{"MyData": "an address"}',
    '{"MyData": "1234"}'
])
...
def oneOfBare(markdown_examples):
    builder = pjs.ObjectBuilder(markdown_examples['OneOfBare'], resolved=markdown_examples)
    return builder.classes['Oneofbare']


@pytest.mark.parametrize('json_object', [
    '{"MyAddress": "an address"}',
    '{"firstName": "John", "lastName": "Winnebago"}'
])
